...
- two public API changes to LaError and det_sign_exact.

  - `LaError::NonFinite { pivot_col }` renamed to `LaError::NonFinite { col }`
    to reflect that NonFinite errors are not always pivot-related (e.g.
    det_sign_exact, det).
  - `Matrix::det_sign_exact()` now returns `Result<i8, LaError>` instead
    of `i8`. Returns `Err(LaError::NonFinite { col })` for NaN/infinite
    entries instead of panicking. This aligns with lu(), ldlt(), and det().
  - When det_direct() overflows to infinity but all entries are finite,
    the fast filter is now skipped gracefully and Bareiss computes the
    correct exact sign (previously panicked).
